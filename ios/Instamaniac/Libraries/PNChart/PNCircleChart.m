...
- (instancetype)initWithFrame:(CGRect)frame total:(NSNumber *)total current:(NSNumber *)current clockwise:(BOOL)clockwise {
...
- (instancetype)initWithFrame:(CGRect)frame total:(NSNumber *)total current:(NSNumber *)current clockwise:(BOOL)clockwise shadow:(BOOL)hasBackgroundShadow shadowColor:(UIColor *)backgroundShadowColor {
    return [self initWithFrame:frame
                         total:total
                       current:current
                     clockwise:clockwise
...
- (instancetype)initWithFrame:(CGRect)frame total:(NSNumber *)total current:(NSNumber *)current clockwise:(BOOL)clockwise shadow:(BOOL)hasBackgroundShadow shadowColor:(UIColor *)backgroundShadowColor displayCountingLabel:(BOOL)displayCountingLabel {
    return [self initWithFrame:frame
                         total:total
                       current:current
                     clockwise:clockwise
...
displayCountingLabel:(BOOL)displayCountingLabel
  overrideLineWidth:(NSNumber *)overrideLineWidth {
    ...
    CGFloat startAngle = clockwise ? -90 : 270;
    ...
    UIBezierPath *circlePath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.frame.size.width/2, self.frame.size.height/2)
                                                              radius:(CGFloat)(self.frame.size.height * 0.5) - ([_lineWidth floatValue]/2)
                                                          startAngle:(CGFloat)DEGREES_TO_RADIANS(startAngle)
                                                            endAngle:(CGFloat)DEGREES_TO_RADIANS(endAngle)
                                                           clockwise:clockwise];
    ...
    _circleBackground.strokeEnd   = 1;
    _circleBackground.zPosition   = -1;

    [self.layer addSublayer:_circle];
    [self.layer addSublayer:_circleBackground];

    _countingLabel = [[UICountingLabel alloc] initWithFrame:CGRectMake(0, 0, 100, 50)];
    [_countingLabel setTextAlignment:NSTextAlignmentCenter];
    [_countingLabel setFont:[UIFont boldSystemFontOfSize:16]];
    [_countingLabel setTextColor:[UIColor grayColor]];
    [_countingLabel setBackgroundColor:[UIColor clearColor]];
    [_countingLabel setCenter:CGPointMake(self.frame.size.width/2, self.frame.size.height/2)];
...
- (void)strokeChart {
    ...
    [_countingLabel countFrom:0 to:(float)([_current floatValue]/([_total floatValue]/100)) withDuration:self.duration];
...
- (void)updateChartByCurrent:(NSNumber *)current byTotal:(NSNumber *)total {
    ...
    if (_displayCountingLabel) {
        [self.countingLabel countFrom:fminf([_current floatValue], [_total floatValue]) to:fminf([current floatValue], [total floatValue]) withDuration:self.duration];
    }
